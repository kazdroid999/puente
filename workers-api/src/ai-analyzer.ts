...
## ★ Micro SaaS 技術スコープ制限（重要）
以下の範囲内のみ対応可能。スコープ外の機能が含まれる場合は scope_ok=false とし、理由を明記:
...
**対応不可（scope_ok=false）:**
- OAuth認証が必要な外部API連携（Google API, Slack API, Notion API, LINE API等）
- リアルタイム通信（WebSocket, Server-Sent Events, チャット）
...
## スコアリング基準
以下の3軸で各0〜100点のスコアを付け、理由も記載:
- feasibility (実現可能性): 3日で開発可能か、技術的難易度、既存ライブラリ/API活用度
- profitability (収益性): 市場規模、課金モデルの妥当性、顧客獲得コスト
- technical_difficulty (技術難易度 ※低いほど高スコア): シンプルなCRUD=高スコア、複雑な処理=低スコア

total_score = (feasibility + profitability + technical_difficulty) / 3 で算出（小数点以下切り捨て）

## 3段階判定基準
- total_score >= 85 かつ scope_ok=true: "auto_dev"（自動開発開始）
- 60 <= total_score < 85: "needs_improvement"（改善提案を返す）
- total_score < 60: "rejected"（却下）
- scope_ok=false の場合: 必ず "needs_improvement" 以下とする（スコアに関わらず auto_dev にしない）

## improvement_suggestions（needs_improvement の場合必須）
スコアが60〜84の場合、または scope_ok=false の場合、具体的な改善提案を3〜5個記載:
- スコアを85以上に引き上げるための具体的なアドバイス
- スコープ外の機能をスコープ内に置き換える代替案
- ターゲットやペインの絞り込み提案
